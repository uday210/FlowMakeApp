...
import {
  ...
  Layers, User, ChevronDown, ChevronUp, Stamp, X,
} from "lucide-react";
      ...
      <div className="bg-white rounded-2xl shadow-2xl border border-gray-200 w-full max-w-sm mx-4 overflow-hidden">
        ...
        <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-100">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-gray-900">Add Watermark</p>
        ...
        <div className="flex items-center gap-2 px-5 py-4 border-t border-gray-100 bg-gray-50">
          ...
          >
            {activeText ? "Apply & Upload" : "Upload"}
          </button>
        </div>
      </div>
    </div>
...
export default function DocumentsPage() {
  ...
  const [watermarkDialog, setWatermarkDialog] = useState<string | null>(null); // file name when open
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setWatermarkDialog(file.name.replace(/\.pdf$/i, ""));
    if (fileRef.current) fileRef.current.value = "";
  };

  const doUpload = async (file: File, watermark: string) => {
    setUploadError("");
    setUploading(true);
    try {
  ...
  const handleWatermarkConfirm = (watermark: string) => {
    const file = pendingFileRef.current;
    setWatermarkDialog(null);
    pendingFileRef.current = null;
    if (file) doUpload(file, watermark);
  };
